...
class TestSendCommand:
    ...
    async def test_send_command_with_params(self, client: GodotWebSocketClient) -> None:
        ...
        mock_ws.recv = AsyncMock(return_value=json.dumps({
            "id": 1,
            "result": {"tick": 100, "elapsed_ms": 5.2},
        }))
        client._ws = mock_ws

        result = await client.send_command("advance_ticks", {"count": 100})

        assert result["tick"] == 100
        sent_data = json.loads(mock_ws.send.call_args[0][0])
        assert sent_data["params"] == {"count": 100}

    async def test_send_command_error_response(self, client: GodotWebSocketClient) -> None:
        mock_ws = AsyncMock()
        mock_ws.recv = AsyncMock(return_value=json.dumps({
            "id": 1,
            "error": {"code": -1, "message": "Entity not found: 999"},
        }))
        client._ws = mock_ws

        with pytest.raises(RuntimeError, match="Entity not found: 999"):
            await client.send_command("get_entity", {"id": 999})
